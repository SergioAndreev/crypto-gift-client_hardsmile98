...
function Modal({ children, isOpen, onClose }: ModalProps) {
  ...
  return (
    <div
      ...
      className={`transition-all duration-[0.4s] h-[100%] fixed inset-0 bg-black/50 z-50 ${isOpen ? 'opacity-1 visible' : 'opacity-0 invisible'}`}
    >
      <div
        className={`fixed bottom-0 left-0 right-0 bg-bg-secondary-light dark:bg-bg-primary-dark rounded-t-2xl z-100 transform transition-all duration-[0.4s] ease-out overflow-y-auto ${isOpen ? 'visible translate-y-0' : 'invisible translate-y-[100%]'}`}
      >
